...
import { FaTimes, FaSpinner, FaUser, FaEnvelope, FaPhone, FaMapMarkerAlt, FaBuilding, FaFileAlt, FaTags, FaFlag, FaCalendarAlt, FaStickyNote, FaPaperclip, FaTrash } from 'react-icons/fa';
...
const EditLetterModal = ({ letter, onClose, categories = ['Education', 'Health', 'Infrastructure', 'Business', 'Policy', 'Environment', 'Social Welfare', 'Public Safety', 'Transportation', 'Utilities', 'Other'], priorities = ['High', 'Medium', 'Low'], statuses = ['Draft', 'Sent', 'Awaiting Response', 'Response Received', 'Closed'] }) => {
        ...
        <div className="flex justify-between items-center px-4 sm:px-6 lg:px-8 py-4 sm:py-6 border-b border-gray-100 bg-gradient-to-r from-green-50 to-emerald-50 rounded-t-2xl flex-shrink-0">
          <div className="flex items-center gap-2 sm:gap-3 lg:gap-4">
            <div className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl flex items-center justify-center shadow-lg">
              <FaFileAlt className="text-white text-sm sm:text-base lg:text-lg" />
            </div>
            <div>
              <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900">Edit Letter</h2>
              <p className="text-xs sm:text-sm text-gray-600 hidden sm:block">Update letter information</p>
            </div>
          </div>
          <button
            onClick={onClose}
